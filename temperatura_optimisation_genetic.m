...
v0 = round([20.27; 18.4]); % values from new optimisation, first go
...
%options = psoptimset('TolFun',1e-4,'Cache','off','PlotFcns',@psplotbestf,'Display','diagnose','MeshRotate','on','CompletePoll','off','PollMethod','MADSPositiveBasisNp1','MeshAccelerator','on');%'InitialMeshSize',4);
options = gaoptimset('Display','diagnose','PlotFcns',{@gaplotgenealogy,@gaplotscores,@gaplotselection,@gaplotbestindiv},'PopulationSize',25,'Generations',20);
...
Aeq = [];
beq = [];
lb = [0.4*pi/Np*bore, (dia.statorlam-bore)/4];% Idens*0.5];  % boundary conditions
ub = [0.7*pi/Np*bore, (dia.statorlam-bore)/2*0.9];% 25];
%[x, fval, exitflag, output] =
%patternsearch(fun,v0,A,b,Aeq,beq,lb,ub,nonlcon,options);

[x, fval, exitflag, output, population, scores] = ga(fun,2,A,b,Aeq,beq,lb,ub,[],options);
